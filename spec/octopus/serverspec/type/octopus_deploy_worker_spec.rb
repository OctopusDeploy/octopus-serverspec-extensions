...
describe OctopusDeployWorker do
  ...
  it "does not throw if the tentacle executable doesn't exist" do
    ...
    worker = OctopusDeployWorker.new("https://someserver.com", "API-someapikey", "Worker1")
    expect(worker.exists?).to be false
  end
  ...
  context "Server supports spaces" do
    ...
    context ".registered_with_the_server" do
      ...
      it "can resolve a known worker in space 2" do
        ...
        existent_worker = OctopusDeployWorker.new('https://octopus.example.com', 'API-1234567890', 'VAGRANTBOX2', 'Spaces-2')
        expect(existent_worker.registered_with_the_server?).to be true
      end
    end
